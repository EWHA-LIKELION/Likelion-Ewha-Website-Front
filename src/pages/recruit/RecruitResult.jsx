      ...
      if (roundName === "1차") {
        return (
          <>
            ...
            을 안내드립니다.
            <br />
            제출해 주신 지원서는 신중하고 꼼꼼하게 검토되었으며,
            <br />그 과정에서 {applicantName}님의 열정과 가능성을 확인할 수
            있었습니다.
            <br />
            <br />
            멋쟁이사자처럼에 관심을 가져주셔서 감사드리며,
            <br />
  ...
  return (
    <Wrapper>
      <ResultCard $status={resultData.status}>
        <ResultWrapper>
          <ResultTitle className={isMobile ? "point-kor-h4" : "point-kor-h2"}>
            {typeof config.title === "function"
              ? config.title(roundName, resultData.applicantName)
              : config.title}
          </ResultTitle>
          <ResultMessage className={isMobile ? "body-regular" : "h5-medium"}>
            {config.getMessage(
              roundName,
        ...
        {resultData.status === "pass" &&
          ...
          resultData.nextStep && (
            <NextStepBox>
              <NextStepTitle className="h5-bold">
                {resultData.nextStep.title}
              </NextStepTitle>
              <NextStepInfo>
                <InfoRow>
                  <Label className="body-bold">일시</Label>
                  <Value className="body-regular">
                    {resultData.nextStep.date} {resultData.nextStep.time}
                  </Value>
                </InfoRow>
                <InfoRow>
                  <Label className="body-bold">장소</Label>
                  <Value className="body-regular">
                    {resultData.nextStep.location}
                  </Value>
                </InfoRow>
              </NextStepInfo>
            </NextStepBox>
